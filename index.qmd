...
np.random.seed(42)
buy_in = 30_000
n_years = 40   # play annually until age 75 (e.g., from 35 to 75)
balances = [buy_in]
balance = buy_in
for _ in range(n_years):
    if np.random.random() < 0.5:   # heads: +50%
        balance *= 1.50
    else:                           # tails: -40%
        balance *= 0.60
...
ax.plot(years, balances, color="steelblue", linewidth=1.5)
ax.axhline(y=buy_in, color="gray", linestyle="--", alpha=0.7, label=f"Buy-in ${buy_in:,}")
ax.set_xlabel("Year")
ax.set_ylabel("Account balance ($)")
ax.set_title("Account Balance Over One Simulation Run")
ax.legend()
...
**Narrative:** In this single run (seed 42), the account starts at $30,000 and evolves year by year according to the coin flips. The path is volatile: some years the balance grows by 50%, others it shrinks by 40%. In this particular simulation, the final balance after 40 years is **$`{python} format(final_balance, ',')`**.

**Would I be satisfied?** That depends entirely on whether this run ended above or below the buy-in. If the final balance is above $30,000, I would be satisfied with *this* outcome—I recovered my stake and gained. If it is below $30,000, I would not be satisfied with this outcome; the same rules that sometimes deliver large gains have, in this single path, left me with less than I put in. One simulation does not tell us whether the *game* is good—only what one possible world looks like. So I would not base my buy-in decision on this single trajectory; I would want to see the distribution of outcomes (Sections 3–4) before deciding.
